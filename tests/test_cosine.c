...
void run_cosine_tests(void) {
    printf("\n======= Running Cosine Tests =======\n");

    hsd_func_f32_f32 func_ptr = hsd_cosine_f32;
    const char* func_name = "hsd_cosine_f32";

    const float vec1[] = {1.0f, 2.0f, 3.0f};
    const float vec2[] = {4.0f, 5.0f, 6.0f};

    run_test_f32(func_ptr, func_name, "Basic Test 1", vec1, vec2, 3, simple_cosine(vec1, vec2, 3),
                 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Parallel Vectors", v_para1, v_para2, 3, 0.0f, 1e-7f);

    const float v_anti1[] = {1.0f, 2.0f, 3.0f};
    const float v_anti2[] = {-1.0f, -2.0f, -3.0f};
    run_test_f32(func_ptr, func_name, "Anti-parallel Vectors", v_anti1, v_anti2, 3, 2.0f, 1e-7f);

    const float v_anti3[] = {1.0f, 2.0f, 3.0f};
    const float v_anti4[] = {-2.0f, -4.0f, -6.0f};
    run_test_f32(func_ptr, func_name, "Anti-parallel Vectors (Scaled)", v_anti3, v_anti4, 3, 2.0f,
                 1e-7f);

    const float v_ortho1[] = {1.0f, 0.0f, 0.0f};
    const float v_ortho2[] = {0.0f, 1.0f, 0.0f};
    run_test_f32(func_ptr, func_name, "Orthogonal Vectors", v_ortho1, v_ortho2, 3, 1.0f, 1e-7f);

    const float v_ortho3[] = {1.0f, 1.0f};
    const float v_ortho4[] = {-1.0f, 1.0f};
    run_test_f32(func_ptr, func_name, "Orthogonal Vectors 2D", v_ortho3, v_ortho4, 2, 1.0f, 1e-7f);

    const float v_zero1[] = {0.0f, 0.0f, 0.0f};
    const float v_non_zero1[] = {3.0f, 4.0f, 0.0f};
    run_test_f32(func_ptr, func_name, "Zero Vector vs Non-Zero", v_zero1, v_non_zero1, 3, 1.0f,
                 1e-7f);
    run_test_f32(func_ptr, func_name, "Non-Zero vs Zero Vector", v_non_zero1, v_zero1, 3, 1.0f,
                 1e-7f);
    ...
    run_test_f32(func_ptr, func_name, "Negative Coordinates", v_neg1, v_neg2, 2,
                 simple_cosine(v_neg1, v_neg2, 2), 1e-7f);
    ...
    run_test_f32(func_ptr, func_name, "One Dimension (Anti-parallel)", v1_a, v1_b, 1, 2.0f, 1e-7f);
    const float v1_c[] = {5.5f};
    const float v1_d[] = {2.0f};
    run_test_f32(func_ptr, func_name, "One Dimension (Parallel)", v1_c, v1_d, 1, 0.0f, 1e-7f);

    const float v3_a[] = {1, 2, 3};
    const float v3_b[] = {4, 5, 6};
    run_test_f32(func_ptr, func_name, "Dimension 3", v3_a, v3_b, 3, simple_cosine(v3_a, v3_b, 3),
                 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 7", v7_a, v7_b, 7, simple_cosine(v7_a, v7_b, 7),
                 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 8", v8_a, v8_b, 8, simple_cosine(v8_a, v8_b, 8),
                 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 9", v9_a, v9_b, 9, simple_cosine(v9_a, v9_b, 9),
                 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 15", v15_a, v15_b, 15,
                 simple_cosine(v15_a, v15_b, 15), 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 16", v16_a, v16_b, 16,
                 simple_cosine(v16_a, v16_b, 16), 1e-6f);
    ...
    run_test_f32(func_ptr, func_name, "Dimension 17", v17_a, v17_b, 17,
                 simple_cosine(v17_a, v17_b, 17), 1e-6f);

    float v_small1[] = {1e-20f, 2e-20f};
    float v_small2[] = {3e-20f, 4e-20f};

    run_test_f32(func_ptr, func_name, "Small Values (Not Quite Parallel)", v_small1, v_small2, 2,
                 1.0f, 1e-7f);
    ...
    hsd_status_t status = hsd_cosine_f32(v_ok, v_ok, 3, NULL);
